...
struct compareClass {
    bool operator()(Class* c1, Class* c2) const {
        if (c1->getCourse() == c2->getCourse()) return c1->getUCode() < c2->getUCode();
...
void readClasses(classSet *classes) {
    ...
    while (getline(file, line)) {
        ...
        (*it)->addSlot(ScheduleSlot(day, start, end, duration, type));
    }
}
...
void listClasses(classSet *classes, studentSet *students) {
    for (auto cl: *classes) {
        ...
        for (const auto &s: cl->getSlots()) {
            cout << "    " << s.getWeekDay() << " - " << s.getStartTime() << " - " << s.getEndTime() << " - "
                 << s.getDuration() << " - " << s.getType() << endl;
        }
...
void studentsFilters(studentSet *students) {
    ...
    while (op1 != 0 && op1 != 1) {
        cout << "Option:"; cin >> op1; cout << endl;
    }

    cout << "Definir intervalo? Nao (0) | Sim (1)" << endl;

    while (op4 != 0 && op4 != 1) {
        cout << "Option:"; cin >> op4; cout << endl;
    }
